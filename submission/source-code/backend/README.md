...
| POST | `/api/v1/hyperlocal/parse-image/` | VLM 이미지 소비패턴 파싱, 미설정/실패 시 fallback |
...
`VLM_API_URL`, `VLM_API_KEY`, `VLM_MODEL` 중 하나라도 없거나 호출/JSON 파싱에 실패하면 `mock_vision_parser` fallback을 반환한다.
...
python manage.py sync_area_to_neo4j --area-id gangnam_station --area-name 강남역 --dry-run
python manage.py sync_area_to_neo4j --area-id gangnam_station --area-name 강남역
```

동기화 명령은 `parse_status=active`인 카드와 활성 혜택만 대상으로 하며, 재실행해도
중복 노드가 생성되지 않도록 `MERGE`를 사용합니다.

Kakao API 키 없이 로컬 그래프 연결만 검증하려면 다음처럼 mock 매장 데이터를
사용할 수 있습니다.

```bash
python manage.py sync_area_to_neo4j --area-id local_mock --area-name 로컬테스트 --use-mock
```

`POST /api/v1/hyperlocal/simulate/` 요청에 `area_id`를 포함하면 Neo4j의
`Area-Store-Category-Card` 관계로 후보 카드를 먼저 좁힌 뒤 Python 추천 코어가
기존 Seul-Score 산식을 계산합니다. Neo4j 조회 실패나 후보 부재 시에는 SQLite
후보 조회로 fallback합니다.
